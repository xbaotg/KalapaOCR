...
from vietocr.model.cc_loss import CCLoss


class Trainer:
    def __init__(
        self, config, pretrained=True, default_loss=False, augmentor=ImgAugTransform(), use_loss_best=False
    ):
        pretrained = False
        self.default_loss = default_loss
        self.config = config
        self.use_loss_best = use_loss_best
    ...
    def train(self):
        ...
        best_acc = 0 if not self.use_loss_best else 999999999
        ...
        for i in range(self.num_iters):
            ...
            if self.iter % self.print_every == 0:
                info = "[Train] iter: {:06d} - loss: {:.3f} - lr: {:.2e} - load time: {:.2f} - gpu time: {:.2f}".format(
                    self.iter,
            ...
            if self.valid_annotation and self.iter % self.valid_every == 0:
                ...
                acc_full_seq, acc_per_char, kalapa = self.precision(self.metrics)

                print("-----------------------")
                print("")
                info = "[Vali] iter: {:06d} - kalapa: {:.4f} - loss: {:.3f} - acc full seq: {:.4f} - acc per char: {:.4f}".format(
                    self.iter, kalapa, val_loss, acc_full_seq, acc_per_char
                )
                print(info)
                self.logger.log(info)
                print("")
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(self.valid_gen):
                ...
                if not self.default_loss:
                    tgt_output_ = tgt_output.reshape(N, -1)
                    target = {
                        "padded_targets": tgt_output_,
                    }
                    outputs_ = outputs.reshape(N, -1, outputs.size(1))
                    char_cont_loss = self.cc_loss(outputs_, target)["loss_cc"]

                total_loss.append(
                    0
                    if isinstance(loss, float)
                    else loss.item() + 0
                    if isinstance(char_cont_loss, float)
                    else char_cont_loss.item()
                )
    ...
    def precision(self, sample=None):
        ...
        kalapa = compute_accuracy(actual_sents, pred_sents, mode="kalapa")

        return acc_full_seq, acc_per_char, kalapa
    ...
    def step(self, batch):
        ...
        total_loss = loss + self.lambda_cc * char_cont_loss
