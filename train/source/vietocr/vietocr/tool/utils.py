...
def download(url, quiet=False):
    ...
    if os.path.exists(full_path):
        print(
            "Model /mlcv/WorkingSpace/Personals/thuyendt/research/kapala/gt_v2.csvweight {} exsits. Ignore download!".format(
                full_path
            )
        )
        return full_path
...
def compute_accuracy(ground_truth, predictions, mode="full_sequence"):
    ...
    if mode == "per_char":
        ...
        for index, label in enumerate(ground_truth):
            prediction = predictions[index]
            if len(prediction) and len(label):
                if prediction == label:
                    score += 1
                else:
                    score += max(
                        0, 1 - 1.5 ** editdistance.eval(prediction, label) / len(label)
                    )
