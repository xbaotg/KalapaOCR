...
if __name__ == '__main__':
    random_StrAug_1 = Random_StrAug(using_aug_types = ['noise', 'camera', 'pattern', 'process', 'weather'],
                                  prob_list = [ 1.0, 0.1, 0.2, 0.2, 0.2])

    
    parser = argparse.ArgumentParser()
    parser.add_argument('--image', default="/mlcv1/WorkingSpace/Personal/baotg/Kalapa/data/base/train/images", help='Load image file')
    parser.add_argument('--results', default="/mlcv1/WorkingSpace/Personal/baotg/Kalapa/argument_data/random_argument", help='Folder for augmented image files')
    ...
    img_list = glob.glob(os.path.join(opt.image,"*/*.jpg"))
    # print(img_list)
    for img_path in tqdm.tqdm(img_list):
        ...
        result = cv2.cvtColor(np.array(augmented_img_1), cv2.COLOR_RGB2BGR)
        # print(os.path.join(opt.results, "/".join(img_path.split('/')[-2:])))
        if not os.path.exists(os.path.join(opt.results, img_path.split('/')[-2])):
            os.makedirs(os.path.join(opt.results, img_path.split('/')[-2]))
        cv2.imwrite(os.path.join(opt.results, "/".join(img_path.split('/')[-2:])),result)
        # input()
