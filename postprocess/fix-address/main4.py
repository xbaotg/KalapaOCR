...
def post_process(predicted):
        ...
        for country, info_country in possible_country.items():
            ...
            for ori_k in dict_country_no_prefix[country].keys():
                ...
                for i in range(len(text) - len(split_k) + 1):
                    ...
                    start_position = len(" ".join(split_text[:i])) + (1 if i > 0 else 0)
            ...
            for city in possible_country[k]["match"].keys():
                ...
                if "match" in possible_country[k]["match"][city].keys():
                    for phuong in possible_country[k]["match"][city]["match"]:
                        left_pos = possible_country[k]["match"][city]["match"][phuong][
                            "info"
                        ][2]
                        ...
                        if k in street_data.keys() and city in street_data[k].keys():
                            for p_street in street_data[k][city]:
                                ...
                                for i in range(len(text) - len(split_k) + 1):
                                    ...
                                    end_position = (
                                        len(" ".join(split_text[: i + len(split_k)]))
                                        - 1
                                    )
                                    ...
                                    cur_text = " ".join(
                                        split_text[i : i + len(split_k)]
                                    )

                                    if any(t in cur_text for t in "0123456789"):
                                        continue

                                    cur_score = levenshtein_distance(
                                        t_k, cur_text
                                    ) / max(1, len(t_k) // diff_between_edit)
                                    ...
                                    if cur_score <= 1:
                                        ...
                                        if (
                                            abs(last_update_score - cur_score)
                                            <= allowed_diff
                                        ):
                                            if (
                                                "match"
                                                not in possible_country[k]["match"][
                                                    city
                                                ]["match"][phuong]
                                            ):
                                                possible_country[k]["match"][city][
                                                    "match"
                                                ][phuong]["match"] = {}
        ...
        if "match" in possible_country[k]:
            for city in possible_country[k]["match"].keys():
                if "match" in possible_country[k]["match"][city]:
                    for street in possible_country[k]["match"][city]["match"].keys():
                        if (
                            "match"
                            in possible_country[k]["match"][city]["match"][street]
                        ):
                            for p_street in possible_country[k]["match"][city]["match"][
                                street
                            ]["match"].keys():
                                results.append(
                                    (
            ...
            results.append(
                (
                    1 + 1000 - possible_country[k]["info"][1],
    ...
    if len(results):
        ...
        for idx in range(len(label)):
            ...
            if (
                len(label[idx - 1]) == 1
                and label[idx].isnumeric()
                and not label[idx - 1].isnumeric()
            ):
...
print(post_process("Ấp Sác Song Ngọc Thành Giồng Ring"))