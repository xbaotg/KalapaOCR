...
for c in ignore_chars.split(","):
    for c2 in ignore_chars.split(","):
        if c != c2:
            ignore.append((c + c2, c2 + c))

SAC = "ú ứ é ế ó ố ớ á ắ ấ í ý" # 0
HUYEN = "ù ừ è ề ò ồ ờ à ằ ầ ì ỳ" # 1
HOI = "ủ ử ẻ ể ỏ ổ ở ả ẳ ẩ ỉ ỷ" # 2
NGA = "ũ ữ ẽ ễ õ ỗ ỡ ã ẵ ẫ ĩ ỹ" # 3
NANG = "ụ ự ẹ ệ ọ ộ ợ ạ ặ ậ ị ỵ" # 4


def get_type(ch):
    if ch in SAC: return 0
    elif ch in HUYEN: return 1
    elif ch in HOI: return 2
    elif ch in NGA: return 3
    elif ch in NANG: return 4
    else: return -1
            ...
            for z in t[2]:
                ...
                if len(matched_text_uni_split) == len(replace_text_uni_split):
                    for i in range(len(matched_text_uni_split)):
                        if matched_text_uni_split[i] == replace_text_uni_split[i]:
                            for k in ignore:
                                if k[0] in matched_text_split[i] and k[1] in replace_text_split[i]:
                                    print("K: ", k, matched_text_uni_split[i], replace_text_uni_split[i])
                                    replace_text_split[i] = matched_text_split[i]
                                    break
